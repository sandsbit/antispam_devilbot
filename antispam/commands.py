...
def hhelp(update, context, custom_first_line: Optional[str] = None):
    ...
    help_ += 'Сей прекрасный бот (пока что) делает лишь одну вещь - позволяет ' \
             'вам запретить участникам чата упоминать вас. Для этого есть команда' \
             ' /dont_disturb_me (запустите еще раз, что бы отменить). Если кто-то ' \
             'вас упомянет, то он получит предупреждение или мут на время.\n' \
             '/list - список всех, кого нельзя упоминать\n' \
             '/top - ТОП-5 нарушителей\n' \
             'Про бота: \n' \
             '/help - эта помощь\n' \
