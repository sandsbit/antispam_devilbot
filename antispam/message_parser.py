...
def handle_mention_violation(update, context):
    ...
    for entity in update.message.entities:
        if entity.type == 'mention':
            ...
            mentions.append(text[b+1:b+len_])
    ...
    for bm in banned_mentions:
        try:
            banned_mentions_dict[UsernamesManager().get_username_by_id(bm).lower()] = bm
        except NoSuchUser:
            pass
    ...
    try:
        ...
        for mention in mentions:
            if mention.lower() in banned_mentions_dict.keys() and mention != username:
                violations_id.append(banned_mentions_dict[mention.lower()])

        if len(violations_id) != 0:
            sanctions = vm.register_violation(chat_id, user_id, violations_id)

            mention_list = ', '.join(set([UsernamesManager().get_username_by_id(id_) for id_ in violations_id]))
            ...
            if sanctions[0] == Sanction.WARNING:
                ...
                message += f'За это @{username} получит мут на пять минут!'
                ...
                message += f'За это @{username} получит мут на час!'

                context.bot.restrict_chat_member(chat_id=chat_id, user_id=user_id, permissions=ChatPermissions(False),
                                                 until_date=int(time.time()) + 3600)
            elif sanctions[0] == Sanction.MUTE_DAY:
                message += f'За это @{username} получит мут на день!'
